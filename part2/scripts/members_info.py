...
while True:
    url = 'https://api.vk.com/method/groups.getMembers?fields=name,sex,bdate,city,universities&access_token='\
          + access_token + '&count=1000&offset='\
          + str(offset) + '&group_id=' +\
          str(publics_list['public_id'][0])
    try:
        members = get_json(url)
    except:
        print ('failed getting members')
